...
func TestBitBuilder_FirstLow(t *testing.T) {
	...
	bb.AppendNBit(1, 1)
	bb.AppendNBit(18, 1)
	require.Equal(t, []byte{0xF4, 0x7F, 0x15, 0x01, 0x02, 0xFF, 0xFF, 0x07}, bb.dump())
	require.Equal(t, 59, bb.Len())

	bb.PadWithBit(1)
	require.Equal(t, []byte{0xF4, 0x7F, 0x15, 0x01, 0x02, 0xFF, 0xFF, 0xFF}, bb.dump())
...
func TestBitBuilder_FirstHigh(t *testing.T) {
	...
	bb.AppendNBit(1, 1)
	bb.AppendNBit(18, 1)
	require.Equal(t, []byte{0x2F, 0xFD, 0x58, 0x01, 0x02, 0xFF, 0xFF, 0xE0}, bb.dump())
	require.Equal(t, 59, bb.Len())

	bb.PadWithBit(1)
	require.Equal(t, []byte{0x2F, 0xFD, 0x58, 0x01, 0x02, 0xFF, 0xFF, 0xFF}, bb.dump())
	require.Equal(t, 64, bb.Len())
