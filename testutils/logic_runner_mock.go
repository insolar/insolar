...
type mLogicRunnerMockValidate struct {
	mock             *LogicRunnerMock
	mockExpectations *LogicRunnerMockValidateParams
}
...
func (m *LogicRunnerMock) Validate(p context.Context, p1 core.RecordRef, p2 core.Pulse, p3 core.CaseBind) (r int, r1 error) {
	...
	if m.ValidateMock.mockExpectations != nil {
		testify_assert.Equal(m.t, *m.ValidateMock.mockExpectations, LogicRunnerMockValidateParams{p, p1, p2, p3},
			"LogicRunner.Validate got unexpected parameters")
	...
	if m.ValidateFunc == nil {
		m.t.Fatal("Unexpected call to LogicRunnerMock.Validate")
		return
	}
...
func (m *LogicRunnerMock) ValidateCallCounters() {
	...
	if m.ValidateFunc != nil && atomic.LoadUint64(&m.ValidateCounter) == 0 {
...
func (m *LogicRunnerMock) MinimockWait(timeout time.Duration) {
	...
	for {
		...
		ok = ok && (m.ValidateFunc == nil || atomic.LoadUint64(&m.ValidateCounter) > 0)
