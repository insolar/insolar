...
func TestBaseConsensus_exchangeDataWithOtherParticipants(t *testing.T) {
	...
	assert.Equal(t, 1, len(c.results.getData(participant2.GetID())))
	assert.Equal(t, newActiveNode(12, 0), c.results.getData(participant2.GetID())[0])

	assert.Equal(t, 2, len(c.results.getData(participant5.GetID())))
	assert.Equal(t, newActiveNode(15, 0), c.results.getData(participant5.GetID())[0])
	assert.Equal(t, newActiveNode(25, 0), c.results.getData(participant5.GetID())[1])
}
