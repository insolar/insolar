...
func (rs *RouteSet) Contains(node *RouteNode) bool {
	...
	for _, n := range rs.nodes {
		if node.ID.HashEqual(n.ID.Hash) {
			exists = true
		}
	}
	return exists
...
func (rs *RouteSet) Remove(node *RouteNode) {
	for i, n := range rs.nodes {
		if n.ID.HashEqual(node.ID.Hash) {
...
func (rs *RouteSet) Less(i, j int) bool {
	iDist := getDistance(rs.nodes[i].ID.Hash, rs.comparator)
	jDist := getDistance(rs.nodes[j].ID.Hash, rs.comparator)
