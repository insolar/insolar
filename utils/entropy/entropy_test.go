...
func TestSelectByEntropy_SelectsCorrectElement(t *testing.T) {
	...
	t.Run("selecting 1 value", func(t *testing.T) {
		...
		require.NoError(t, err)
		var selected []insolar.RecordRef
		for _, v := range out {
			selected = append(selected, v.(insolar.RecordRef))
		}

		assert.Equal(t, 1, len(out))
		// Indexes are hard-coded from previously calculated values.
		assert.Equal(t, []insolar.RecordRef{values[37]}, selected)
	})
	...
	t.Run("selecting 3 values", func(t *testing.T) {
		...
		// Indexes are hard-coded from previously calculated values.
		assert.Equal(t, []insolar.RecordRef{values[37], values[22], values[53]}, selected)
	})
}
