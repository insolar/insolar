...
//go:generate minimock -i github.com/insolar/insolar/logicrunner/requestsfetcher.RequestsFetcher -o ./ -s _mock.go -g
...
//go:generate minimock -i github.com/insolar/insolar/logicrunner/requestsfetcher.Broker -o ./ -s _mock.go -g

type Broker interface {
	NoMoreRequestsOnLedger(ctx context.Context)
	IsKnownRequest(ctx context.Context, reqRef insolar.Reference) bool
...
type requestsFetcher struct {
	...
	broker Broker
	am     artifacts.Client
	os     outgoingsender.OutgoingRequestSender
}

func NewRequestsFetcher(
	obj insolar.Reference, am artifacts.Client, br Broker, os outgoingsender.OutgoingRequestSender,
) RequestsFetcher {
...
func (rf *requestsFetcher) fetch(ctx context.Context) error {
	...
	for _, reqRef := range reqRefs {
		...
		case *record.IncomingRequest:
			...
			tr := transcript.NewTranscript(requestCtx, reqRef, *v)
