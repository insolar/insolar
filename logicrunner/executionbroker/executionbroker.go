...
//go:generate minimock -i github.com/insolar/insolar/logicrunner/executionbroker.BrokerI -o ./ -s _mock.go -g

type BrokerI interface {
	AddFreshRequest(ctx context.Context, transcript *transcript.Transcript)
	AddRequestsFromPrevExecutor(ctx context.Context, transcripts ...*transcript.Transcript)
	AddRequestsFromLedger(ctx context.Context, transcripts ...*transcript.Transcript)
	AddAdditionalRequestFromPrevExecutor(ctx context.Context, transcript *transcript.Transcript)
...
type ExecutionBroker struct {
	...
	currentList *currentexecutor.CurrentExecutionList
	...
	requestsFetcher  requestsfetcher.RequestsFetcher
...
func NewExecutionBroker(
	...
	return &ExecutionBroker{
		...
		mutable:     transcriptdequeue.NewTranscriptDequeue(),
		immutable:   transcriptdequeue.NewTranscriptDequeue(),
		finished:    transcriptdequeue.NewTranscriptDequeue(),
...
func (q *ExecutionBroker) getImmutableTask(ctx context.Context) *transcript.Transcript {
	q.stateLock.Lock()
...
func (q *ExecutionBroker) getMutableTask(ctx context.Context) *transcript.Transcript {
	q.stateLock.Lock()
	defer q.stateLock.Unlock()
...
func (q *ExecutionBroker) storeCurrent(ctx context.Context, transcript *transcript.Transcript) {
	err := q.currentList.SetOnce(transcript)
	if err != nil {
...
func (q *ExecutionBroker) releaseTask(_ context.Context, transcript *transcript.Transcript) {
	q.stateLock.Lock()
	defer q.stateLock.Unlock()
...
func (q *ExecutionBroker) finishTask(ctx context.Context, transcript *transcript.Transcript) {
	q.stateLock.Lock()
	defer q.stateLock.Unlock()
...
func (q *ExecutionBroker) processTranscript(ctx context.Context, transcript *transcript.Transcript) bool {
...
func (q *ExecutionBroker) storeWithoutDuplication(ctx context.Context, transcript *transcript.Transcript) bool {
...
func (q *ExecutionBroker) Prepend(ctx context.Context, start bool, transcripts ...*transcript.Transcript) {
...
func (q *ExecutionBroker) Put(ctx context.Context, start bool, transcripts ...*transcript.Transcript) {
	for _, transcript := range transcripts {
		if q.storeWithoutDuplication(ctx, transcript) {
			continue
		}
...
func (q *ExecutionBroker) HasLedgerRequest(_ context.Context) *transcript.Transcript {
...
func (q *ExecutionBroker) GetByReference(_ context.Context, r *insolar.Reference) *transcript.Transcript {
	q.stateLock.Lock()
	defer q.stateLock.Unlock()
...
func (q *ExecutionBroker) startRequestsFetcher(ctx context.Context) {
	if q.requestsFetcher == nil {
		q.requestsFetcher = requestsfetcher.NewRequestsFetcher(q.Ref, q.artifactsManager, q, q.outgoingSender)
	}
...
func (q *ExecutionBroker) AddFreshRequest(
	ctx context.Context, tr *transcript.Transcript,
) {
	q.stateLock.Lock()
	defer q.stateLock.Unlock()
...
func (q *ExecutionBroker) AddRequestsFromPrevExecutor(ctx context.Context, transcripts ...*transcript.Transcript) {
	q.stateLock.Lock()
	defer q.stateLock.Unlock()
...
func (q *ExecutionBroker) AddRequestsFromLedger(ctx context.Context, transcripts ...*transcript.Transcript) {
	q.stateLock.Lock()
	defer q.stateLock.Unlock()
...
func convertQueueToMessageQueue(ctx context.Context, queue []*transcript.Transcript) []message.ExecutionQueueElement {
	...
	for _, elem := range queue {
		mq = append(mq, message.ExecutionQueueElement{
			...
			ServiceData: common.ServiceDataFromContext(elem.Context),
		})
