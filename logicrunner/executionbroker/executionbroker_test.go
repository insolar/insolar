...
import (
	...
	"github.com/insolar/insolar/logicrunner/testutils"
	"github.com/insolar/insolar/logicrunner/transcript"
	instestutils "github.com/insolar/insolar/testutils"
)
...
func (s *ExecutionBrokerSuite) TestPut() {
	...
	rem := requestsexecutor.NewRequestsExecutorMock(s.T())
	rem.ExecuteAndSaveMock.Set(func(_ context.Context, t *transcript.Transcript) (r insolar.Reply, r1 error) {
	...
	tr := transcript.NewTranscript(s.Context, gen.Reference(), record.IncomingRequest{})
	...
	s.True(testutils.WaitOnChannel(waitChannel), "failed to wait until put triggers start of queue processor")
	s.True(testutils.WaitOnChannel(waitChannel), "failed to wait until queue processor'll finish processing")
	s.Require().True(testutils.Wait(ProcessorStatus, b, false))
	s.Empty(waitChannel)
...
func (s *ExecutionBrokerSuite) TestPrepend() {
	...
	tr := transcript.NewTranscript(s.Context, reqRef1, record.IncomingRequest{})
	b.Prepend(s.Context, false, tr)
	s.Equal(b.mutable.Length(), 1)

	reqRef2 := gen.Reference()
	tr = transcript.NewTranscript(s.Context, reqRef2, record.IncomingRequest{})
	b.Prepend(s.Context, true, tr)
	s.Require().True(testutils.WaitOnChannel(waitChannel), "failed to wait until put triggers start of queue processor")
	s.Require().True(testutils.WaitOnChannel(waitChannel), "failed to wait until queue processor'll finish processing")
	s.Require().True(testutils.Wait(ProcessorStatus, b, false))
...
func (s *ExecutionBrokerSuite) TestImmutable_NotPending() {
	...
	tr := transcript.NewTranscript(s.Context, gen.Reference(), record.IncomingRequest{Immutable: true})

	b.Prepend(s.Context, false, tr)
	s.Equal(b.immutable.Length(), 1)

	reqRef2 := gen.Reference()
	tr = transcript.NewTranscript(s.Context, reqRef2, record.IncomingRequest{Immutable: true})

	b.Prepend(s.Context, true, tr)
	s.Require().True(testutils.WaitOnChannel(waitImmutableChannel), "failed to wait while processing is finished")
	s.Require().True(testutils.WaitOnChannel(waitImmutableChannel), "failed to wait while processing is finished")
	s.Require().True(testutils.Wait(ProcessorStatus, b, false))
...
func (s *ExecutionBrokerSuite) TestImmutable_InPending() {
	...
	b.Prepend(s.Context, false, tr)
	s.Require().True(testutils.Wait(ImmutableCount, b, 1), "failed to wait until immutable was put")
	s.Require().True(testutils.Wait(ProcessorStatus, b, false))
	...
	s.Require().True(testutils.Wait(ImmutableCount, b, 2), "failed to wait until immutable was put")
	s.Require().True(testutils.Wait(ProcessorStatus, b, false))
	s.Require().Empty(waitMutableChannel)
...
func (s *ExecutionBrokerSuite) TestRotate() {
	...
	for i := 0; i < 4; i++ {
		b.stateLock.Lock()
		b.immutable.Push(&transcript.Transcript{})
		b.mutable.Push(&transcript.Transcript{})
		b.stateLock.Unlock()
	}
...
func (s *ExecutionBrokerSuite) TestDeduplication() {
	...
	reqRef1 := gen.Reference()
	b.Put(s.Context, false, transcript.NewTranscript(s.Context, reqRef1, record.IncomingRequest{})) // no duplication
	s.Equal(b.mutable.Length(), 1)

	b.Put(s.Context, false, transcript.NewTranscript(s.Context, reqRef1, record.IncomingRequest{})) // duplication
	s.Equal(b.mutable.Length(), 1)
}
	...
	}{
		{
			...
			mocks: func(t minimock.Tester) *ExecutionBroker {
				...
				broker := &ExecutionBroker{
					...
					messageBus: instestutils.NewMessageBusMock(t).SendMock.Return(&reply.OK{}, nil),
				}
...
func TestExecutionBroker_ExecuteImmutable(t *testing.T) {
	...
	objectRef := gen.Reference()
	re := requestsexecutor.NewRequestsExecutorMock(mc)
	...
	immutableTranscript1 := transcript.NewTranscript(ctx, immutableRequestRef1, immutableRequest1)
...
func TestExecutionBroker_OnPulse(t *testing.T) {
	randTranscript := func(ctx context.Context) *transcript.Transcript {
		reqRef := gen.Reference()
		return transcript.NewTranscript(ctx, reqRef, record.IncomingRequest{})
	}
	...
	table := []struct {
	...
	}{
		{
			...
			mocks: func(ctx context.Context, t minimock.Tester) *ExecutionBroker {
				...
				// fetcher is stopped
				broker.requestsFetcher = requestsfetcher.NewRequestsFetcherMock(t).AbortMock.Return()
		...
		},
		{
			...
			mocks: func(ctx context.Context, t minimock.Tester) *ExecutionBroker {
				objectRef := gen.Reference()
				broker := NewExecutionBroker(objectRef, nil, nil, nil, nil, nil, nil, nil, nil)
				broker.requestsFetcher = requestsfetcher.NewRequestsFetcherMock(t)
				broker.currentList.SetOnce(randTranscript(ctx))
				return broker
			},
			numberOfMessages: 0,
...
func TestExecutionBroker_AddFreshRequestWithOnPulse(t *testing.T) {
	...
	table := []struct {
		...
		},
		{
			...
			mocks: func(ctx context.Context, t minimock.Tester) (*ExecutionBroker, *[]insolar.Message) {
				...
				re := requestsexecutor.NewRequestsExecutorMock(t).
				...
				var msgs []insolar.Message
				re.ExecuteAndSaveMock.Set(func(ctx context.Context, tr *transcript.Transcript) (insolar.Reply, error) {
					msgs = broker.OnPulse(ctx, false)
					return &reply.OK{}, nil
				})
				return broker, &msgs
	...
	for _, test := range table {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			reqRef := gen.Reference()
			broker.AddFreshRequest(ctx, transcript.NewTranscript(ctx, reqRef, record.IncomingRequest{}))
