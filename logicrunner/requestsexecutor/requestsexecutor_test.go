	...
	table := []struct {
	...
	}{
		{
			...
			le: logicexecutor.NewLogicExecutorMock(mc).
				ExecuteMock.
				Return(
					&logicexecutor.RequestResult{
						SideEffectType:     artifacts.RequestSideEffectActivate,
						ObjectReferenceRef: requestRef,
					},
					nil,
				),
...
func TestRequestsExecutor_Execute(t *testing.T) {
	...
	table := []struct {
	...
	}{
		{
			...
			le:     logicexecutor.NewLogicExecutorMock(mc).ExecuteMock.Return(&logicexecutor.RequestResult{SideEffectType: artifacts.RequestSideEffectActivate}, nil),
			result: &logicexecutor.RequestResult{SideEffectType: artifacts.RequestSideEffectActivate},
		},
		{
			name: "success, method",
			transcript: &transcript.Transcript{
...
func TestRequestsExecutor_Save(t *testing.T) {
	...
	table := []struct {
		name       string
		result     *logicexecutor.RequestResult
		transcript *transcript.Transcript
		am         artifacts.Client
		error      bool
		reply      insolar.Reply
		...
		},
		{
			...
			result: &logicexecutor.RequestResult{
				SideEffectType: artifacts.RequestSideEffectDeactivate, ResultData: []byte{1, 2, 3}},
		...
		},
		{
			...
			result: &logicexecutor.RequestResult{SideEffectType: artifacts.RequestSideEffectAmend, Memory: []byte{3, 2, 1}, ResultData: []byte{1, 2, 3}},
			am:     artifacts.NewClientMock(mc).RegisterResultMock.Return(errors.New("some")),
			error:  true,
		},
		{
		...
		},
		{
			...
			result: &logicexecutor.RequestResult{SideEffectType: artifacts.RequestSideEffectNone, ResultData: []byte{1, 2, 3}},
			am:     artifacts.NewClientMock(mc).RegisterResultMock.Return(errors.New("some")),
			error:  true,
		},
	}
