...
func (s *ExecutionRegistrySuite) TestRegister() {
	...
	err := Register(ctx, firstTranscript)
	s.NoError(err)
	...
	// successful archiving
	err = Register(ctx, s.genTranscriptForObject())
	s.NoError(err)
...
func (s *ExecutionRegistrySuite) TestGetActiveTranscript() {
	...
	{ // have (put before)
		s.NotNil(GetActiveTranscript(T.RequestRef))
	}
	{ // don't have
		s.Nil(GetActiveTranscript(gen.Reference()))
	}

	Done(T)
	{ // don't have (done task)
		s.Nil(GetActiveTranscript(T.RequestRef))
	}
}
