...
test_slow: ## run tests with slowtest tag
	CGO_ENABLED=1 go test $(TEST_ARGS) -tags slowtest ./logicrunner/... ./server/internal/... ./ledger/light/integration/... ./ledger/heavy/executor/intergration/...  ./ledger/heavy/integration/...
...
ci_test_slow: ## run slow tests just once, redirects json output to file (CI)
	GOMAXPROCS=$(CI_GOMAXPROCS) CGO_ENABLED=1 \
		go test $(CI_TEST_ARGS) $(TEST_ARGS) -json -v -tags slowtest ./logicrunner/... ./server/internal/... ./ledger/heavy/executor/intergration/...  ./ledger/heavy/integration/... ./ledger/light/integration/... -count 1 | tee -a ci_test_unit.json
