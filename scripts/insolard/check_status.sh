...
do
    ...
    status=$(curl --header "Content-Type:application/json"  --data '{ "jsonrpc": "2.0", "method": "node.getStatus","id": "" }'  "localhost:$port/api/rpc" 2>/dev/null |  python -m json.tool | grep -i NodeState)
    echo "    $status"
    echo
