...
func (rd *RootDomain) RegisterNode(publicKey string, numberOfBootstrapNodes int, majorityRule int, roles []string, ip string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("[ RegisterNode ] Problems with RegisterNode: " + err.Error())
	}
...
func (rd *RootDomain) Authorize() (string, []core.NodeRole, error) {
	...
	if err != nil {
		return "", nil, fmt.Errorf("[ RootDomain::Authorize ] Can't generate private key" + err.Error())
	}
	...
	if err != nil {
		return "", nil, fmt.Errorf("[ RootDomain::Authorize ] Can't sign" + err.Error())
	}

	// Register node
	serPubKey, err := cryptoHelper.ExportPublicKey(&privateKey.PublicKey)
	if err != nil {
		return "", nil, fmt.Errorf("[ RootDomain::Authorize ] Can't export public key" + err.Error())
	}

	rawJSON, err := rd.RegisterNode(serPubKey, 0, 0, []string{"virtual"}, "127.0.0.1")
	if err != nil {
		return "", nil, fmt.Errorf("[ RootDomain::Authorize ] Can't register node" + err.Error())
	}

	nodeRef, err := networkcoordinator.ExtractNodeRef(rawJSON)
	if err != nil {
		return "", nil, fmt.Errorf("[ RootDomain::Authorize ] Can't extract node ref" + err.Error())
	}

	// Validate
	domainRefs, err := rd.GetChildrenTyped(nodedomain.ClassReference)
	if err != nil {
		return "", nil, fmt.Errorf("[ RootDomain::Authorize ] Can't get children" + err.Error())
	}
	nd := nodedomain.GetObject(domainRefs[0])
...
func (rd *RootDomain) CreateMember(name string, key string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("[ CreateMember ] Can't save as child" + err.Error())
	}

	wHolder := wallet.New(1000)
	_, err = wHolder.AsDelegate(m.GetReference())
	if err != nil {
		return "", fmt.Errorf("[ CreateMember ] Can't save as delegate" + err.Error())
	}
...
func (rd *RootDomain) GetBalance(reference string) (uint, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("[ GetBalance ] Can't get implementation" + err.Error())
	}
...
func (rd *RootDomain) SendMoney(from string, to string, amount uint) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("[ SendMoney ] Can't get implementation" + err.Error())
	}
...
func (rd *RootDomain) getUserInfoMap(m *member.Member) (map[string]interface{}, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("[ getUserInfoMap ] Can't get implementation" + err.Error())
	}

	name, err := m.GetName()
	if err != nil {
		return nil, fmt.Errorf("[ getUserInfoMap ] Can't get name" + err.Error())
	}

	balance, err := w.GetTotalBalance()
	if err != nil {
		return nil, fmt.Errorf("[ getUserInfoMap ] Can't get total balance" + err.Error())
	}
...
func (rd *RootDomain) DumpUserInfo(reference string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("[ DumpUserInfo ] Problem with making request" + err.Error())
	}
...
func (rd *RootDomain) DumpAllUsers() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("[ DumpUserInfo ] Can't get children" + err.Error())
	}
	for _, cref := range crefs {
		m := member.GetObject(cref)
		userInfo, err := rd.getUserInfoMap(m)
		if err != nil {
			return nil, fmt.Errorf("[ DumpAllUsers ] Problem with making request" + err.Error())
		}
