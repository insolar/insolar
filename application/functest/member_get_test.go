...
func TestMemberGet(t *testing.T) {
	...
	res, err := testutils.SignedRequest(t, launchnet.TestRPCUrlPublic, member2, "member.get", nil)
	require.Nil(t, err)
...
func TestMigrationMemberGet(t *testing.T) {
	...
	res1, err := testutils.SignedRequest(t, launchnet.TestRPCUrlPublic, member1, "member.migrationCreate", nil)
	require.Nil(t, err)
	...
	res2, err := testutils.SignedRequest(t, launchnet.TestRPCUrlPublic, member1, "member.get", nil)
	require.Nil(t, err)
...
func TestMemberGetWrongPublicKey(t *testing.T) {
	member1, _ := newUserWithKeys()
	_, err := testutils.SignedRequestWithEmptyRequestRef(t, launchnet.TestRPCUrlPublic, member1, "member.get", nil)
