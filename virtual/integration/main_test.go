...
func (s *Server) PrepareAndStart() (*Server, error) {
	...
	ctx, logger := inslogger.InitNodeLogger(s.ctx, s.cfg.Log, traceID, "", "virtual")
	s.ctx = ctx
	...
	logicRunner, err := logicrunner.NewLogicRunner(&s.cfg.LogicRunner, IncomingPubSub, ClientBus)
