...
func (s *SetOutgoingRequest) Present(ctx context.Context, f flow.Flow) error {
	...
	jet := proc.NewCheckJet(*request.GetObject().Record(), flow.Pulse(ctx), s.message, passIfNotExecutor)
	s.dep.CheckJet(jet)
	...
	getIndex := proc.NewEnsureIndexWM(*request.GetObject().Record(), objJetID, s.message)
