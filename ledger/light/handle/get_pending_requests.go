...
func NewGetPendingRequests(dep *proc.Dependencies, msg bus.Message, parcel insolar.Parcel) *GetPendingRequests {
...
func (s *GetPendingRequests) Present(ctx context.Context, f flow.Flow) error {
	jet := proc.NewFetchJet(*s.msg.DefaultTarget().Record(), flow.Pulse(ctx), s.message.WatermillMsg)
	s.dep.FetchJet(jet)
	if err := f.Procedure(ctx, jet, false); err != nil {
		return err
	}

	getPendingRequests := proc.NewGetPendingRequests(jet.Result.Jet, s.message.ReplyTo, s.msg, s.reqPulse)
