...
func (m *mLifelineAccessorMockForID) Expect(p context.Context, p1 insolar.ID) *mLifelineAccessorMockForID {
	...
	m.mainExpectation.input = &LifelineAccessorMockForIDInput{p, p1}
	return m
}
...
func (m *mLifelineAccessorMockForID) ExpectOnce(p context.Context, p1 insolar.ID) *LifelineAccessorMockForIDExpectation {
	m.mock.ForIDFunc = nil
	m.mainExpectation = nil

	expectation := &LifelineAccessorMockForIDExpectation{}
	expectation.input = &LifelineAccessorMockForIDInput{p, p1}
...
func (m *mLifelineAccessorMockForID) Set(f func(p context.Context, p1 insolar.ID) (r Lifeline, r1 error)) *LifelineAccessorMock {
	m.mainExpectation = nil
	m.expectationSeries = nil
...
func (m *LifelineAccessorMock) ForID(p context.Context, p1 insolar.ID) (r Lifeline, r1 error) {
	...
	if len(m.ForIDMock.expectationSeries) > 0 {
		if counter > uint64(len(m.ForIDMock.expectationSeries)) {
			m.t.Fatalf("Unexpected call to LifelineAccessorMock.ForID. %v %v", p, p1)
			return
		}

		input := m.ForIDMock.expectationSeries[counter-1].input
		testify_assert.Equal(m.t, *input, LifelineAccessorMockForIDInput{p, p1}, "LifelineAccessor.ForID got unexpected parameters")
