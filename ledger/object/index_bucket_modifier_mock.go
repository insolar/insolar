...
// IndexBucketModifierMock implements github.com/insolar/insolar/ledger/object.IndexModifier
...
// NewIndexBucketModifierMock returns a mock for github.com/insolar/insolar/ledger/object.IndexModifier
...
func (m *IndexBucketModifierMock) SetIndex(p context.Context, p1 insolar.PulseNumber, p2 FilamentIndex) (r error) {
	...
	if len(m.SetBucketMock.expectationSeries) > 0 {
		if counter > uint64(len(m.SetBucketMock.expectationSeries)) {
			m.t.Fatalf("Unexpected call to IndexBucketModifierMock.SetIndex. %v %v %v", p, p1, p2)
			return
		}

		input := m.SetBucketMock.expectationSeries[counter-1].input
		testify_assert.Equal(m.t, *input, IndexBucketModifierMockSetBucketInput{p, p1, p2}, "IndexModifier.SetIndex got unexpected parameters")

		result := m.SetBucketMock.expectationSeries[counter-1].result
		if result == nil {
			m.t.Fatal("No results are set for the IndexBucketModifierMock.SetIndex")
			return
		}
...
// SetBucketMinimockPreCounter returns the value of IndexBucketModifierMock.SetIndex invocations
...
func (m *IndexBucketModifierMock) ValidateCallCounters() {
	...
	if !m.SetBucketFinished() {
		m.t.Fatal("Expected call to IndexBucketModifierMock.SetIndex")
	}
...
func (m *IndexBucketModifierMock) MinimockWait(timeout time.Duration) {
	...
	for {
		...
		case <-timeoutCh:
			...
			if !m.SetBucketFinished() {
				m.t.Error("Expected call to IndexBucketModifierMock.SetIndex")
			}
