...
func (db *DB) SetReplicatedPulse(ctx context.Context, jet core.RecordID, pulsenum core.PulseNumber) error {
	return db.Update(ctx, func(tx *TransactionManager) error {
		k := prefixkeyany(scopeIDSystem, jet[:], []byte{sysReplicatedPulse})
...
func (db *DB) SetHeavySyncedPulse(ctx context.Context, jet core.RecordID, pulsenum core.PulseNumber) error {
	return db.Update(ctx, func(tx *TransactionManager) error {
		return tx.set(ctx, prefixkeyany(scopeIDSystem, jet[:], []byte{sysLastSyncedPulseOnHeavy}), pulsenum.Bytes())
	})
}

// GetHeavySyncedPulse returns last successfuly synced pulse number on heavy node.
func (db *DB) GetHeavySyncedPulse(ctx context.Context, jet core.RecordID) (pn core.PulseNumber, err error) {
	var buf []byte
	buf, err = db.get(ctx, prefixkeyany(scopeIDSystem, jet[:], []byte{sysLastSyncedPulseOnHeavy}))
	if err == nil {
