...
func Test_IDLockTheSame(t *testing.T) {
	tl := newtestlocker()
	id1 := insolar.RecordID{0x0A}
	id2 := insolar.RecordID{0x0A}
...
func Test_IDLockDifferent(t *testing.T) {
	tl := newtestlocker()
	id1 := insolar.NewRecordID(0, []byte{0x0A})
	id2 := insolar.NewRecordID(1, []byte{0x0A})
	end := make(chan bool)
	go func() {
		tl.Lock("lock1", id1)
		tl.Lock("lock2", id2)
...
func (tl *testlock) Lock(name string, id *insolar.RecordID) {
	tl.synclist.Add("before-" + name)
	tl.lock.Lock(id)
}

func (tl *testlock) Unlock(name string, id *insolar.RecordID) {
	tl.synclist.Add("before-" + name)
	tl.lock.Unlock(id)
}
