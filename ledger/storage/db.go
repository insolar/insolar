...
// NewStore returns storage.DB with BadgerDB instance initialized by opts.
// Creates database in provided dir or in current directory if dir parameter is empty.
func NewStore(dir string, opts *badger.Options) (*DB, error) {
...
func (s *DB) GetRecord(ref *record.Reference) (record.Record, error) {
...
func (s *DB) SetRecord(rec record.Record) (*record.Reference, error) {
...
func (s *DB) GetClassIndex(ref *record.Reference) (*index.ClassLifeline, error) {
	tx := s.BeginTransaction(false)
	defer tx.Discard()
...
func (s *DB) SetClassIndex(ref *record.Reference, idx *index.ClassLifeline) error {
	tx := s.BeginTransaction(true)
	defer tx.Discard()
...
func (s *DB) GetObjectIndex(ref *record.Reference) (*index.ObjectLifeline, error) {
	tx := s.BeginTransaction(false)
	defer tx.Discard()
...
func (s *DB) SetObjectIndex(ref *record.Reference, idx *index.ObjectLifeline) error {
	tx := s.BeginTransaction(true)
	defer tx.Discard()
...
func (s *DB) GetDrop(pulse record.PulseNum) (*jetdrop.JetDrop, error) {
...
func (s *DB) SetDrop(pulse record.PulseNum, prevdrop *jetdrop.JetDrop) (*jetdrop.JetDrop, error) {
	s.waitinflight()
...
func (s *DB) GetEntropy(pulse record.PulseNum) ([]byte, error) {
	tx := s.BeginTransaction(false)
	defer tx.Discard()
...
func (s *DB) SetEntropy(pulse record.PulseNum, entropy []byte) error {
	tx := s.BeginTransaction(true)
	defer tx.Discard()
...
func (s *DB) SetCurrentPulse(pulse record.PulseNum) {
	s.currentPulse = pulse
}

// GetCurrentPulse returns current pulse number.
func (s *DB) GetCurrentPulse() record.PulseNum {
...
func (s *DB) BeginTransaction(update bool) *TransactionManager {
	if update {
		s.dropWG.Add(1)
	}
...
func (s *DB) View(fn func(*TransactionManager) error) error {
	tx := s.BeginTransaction(false)
	defer tx.Discard()
	return fn(tx)
}
...
func (s *DB) Update(fn func(*TransactionManager) error) error {
	tx := s.BeginTransaction(true)
	defer tx.Discard()
