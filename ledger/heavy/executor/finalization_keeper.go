...
func (f *FinalizationKeeperDefault) OnPulse(ctx context.Context, current insolar.PulseNumber) error {
	...
	if lastConfirmedPulse <= bottomLevel.PulseNumber {
		...
		inslogger.FromContext(ctx).Panic(fmt.Sprintf("it shouldn't be called. lastConfirmedPulse:%v, bottomLevel.PulseNumber:%v", lastConfirmedPulse, bottomLevel.PulseNumber))
